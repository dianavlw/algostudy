...
:note: this is reference to youtube and blogs i found on line, its mainly to study I take no ownership Im finding which best
       way helps me undersand the problem. :) :D BTW you are doing great you got this!   
          
Problem 11: (date: 09/04/21)          
...
Reference to: https://euler.beerbaronbill.com/en/latest/solutions/11.html
https://www.youtube.com/watch?v=w8is3T5KOfM
...
#DOWNWARD
for i in range(len(grid) - length):
...
for i in range(len(grid) -length): #initial value
  for j in range(length, len(grid[i])):
    for k in range(length):
        current *= grid[i+k][j-k] #pushes along and down, backwards
    #print(current)
    if current > largest_so_far:
      largest_so_far = current
    current = 1 
